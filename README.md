...
| 기능 | 설명 |
|------|------|
| 회원가입 / 로그인 | 이메일 회원가입, 로그인 유지, 비밀번호 재설정 |
| 마이페이지 | 프로필 사진 · 닉네임 수정, 내가 판매 중인 상품과 구매 내역 확인 |

### 2. 상품 판매
| 기능 | 설명 |
|------|------|
| 상품 등록 | 사진 여러 장과 함께 상품 정보(카테고리 · 브랜드 · 사이즈 · 상태 · 가격) 등록 |
| 상품 수정 / 삭제 | 판매 중인 상품 정보 변경, 판매 완료 처리 |

### 3. 상품 탐색
| 기능 | 설명 |
|------|------|
| 검색 | 키워드로 원하는 상품 검색 |
| 상세 필터 | 카테고리 · 가격대 · 사이즈 · 상품 상태를 조합해 조건에 맞는 상품만 조회 |
| 정렬 | 최신순 / 인기순(조회수 + 찜 수 반영)으로 정렬 |
| 홈 화면 | 신상품, 인기 상품 TOP, 랜덤 추천, 보고 있는 상품과 비슷한 연관 상품 노출 |

### 4. 찜
| 기능 | 설명 |
|------|------|
| 찜 추가 / 해제 | 마음에 드는 상품을 하트 버튼으로 저장 · 해제 |
...
| 주문 취소 | 구매자가 취소 요청, 판매자 승인으로 취소 확정 |

### 6. 신뢰 · 안전
| 기능 | 설명 |
|------|------|
| 리뷰 | 거래 완료 후 판매자 리뷰 작성, 판매자 평점 확인 |
| 신고 | 이상한 상품이나 비매너 이용자 신고 |
| 1:1 문의 | 이용 중 궁금한 점 문의하고 답변 확인 |

### 7. 관리자
| 기능 | 설명 |
|------|------|
| 상품 관리 | 부적절한 상품을 가리고(블라인드), 문제 해결 시 다시 노출 |
| 신고 처리 | 접수된 신고를 확인해 반려하거나 제재 확정 |
| 회원 제재 | 악성 이용자에게 경고 · 이용 정지 조치 |
