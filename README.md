...
| 상품 등록·수정·삭제 | 삭제는 soft delete로 처리, 진행 중 거래(결제·배송)가 있으면 삭제 차단 |
| 검색·필터 | 키워드·카테고리·가격·사이즈·컨디션을 조건부 동적 쿼리(파라미터 없으면 조건 제외)로 조합 + 페이징 |
| 정렬 | 최신순·가격순, 인기순은 `조회수 + 찜수 × 3` 점수로 계산 |
| 상세 조회 | 판매자·카테고리 계층·브랜드를 JOIN FETCH 단일 쿼리로 묶어 N+1 제거, 로그인 시 찜 여부 포함 |
| 조회수 증가 | DB에서 직접 `+1` UPDATE로 동시 요청 유실 방지, 세션으로 중복 조회 카운트 차단 |
| 이미지 업로드 | 임시 저장 후 등록 확정 시 `PRD_001_1.jpg`(시퀀스_순번)로 rename·이동, 등록 취소 시 잔여 파일 방지 |
| 홈 화면 API | 신상품·인기 TOP·랜덤·연관 상품, 목록 썸네일 배치 조회로 N+1 방지 |
...
| 찜 등록·취소·목록 | 마이페이지 찜 목록 페이징 |
| 찜 수 동기화 | 상품 찜 카운트를 DB에서 직접 `+1 / -1` UPDATE, 감소 시 0 미만 방지 가드로 동시 요청에도 정합성 유지 |
...
| 판매자 리뷰 목록 | 평균 별점 통계 + 리뷰 목록 함께 반환(비로그인 조회 가능) |
| N+1 방지 | 상품 정보·썸네일 배치 조회 |
...
| 신고 접수·조회 | 신고 등록, 내 신고 내역 조회(페이징) |
| 관리자 처리 | 신고 목록(키워드·사유·상태·기간 필터), 반려·제재 — 상태가 '접수'일 때만 처리해 중복 제재 방지, 제재 생성 + 신고 상태 변경을 한 트랜잭션으로 처리 |
