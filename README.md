...
상품 · 찜 · 관리자(상품·신고) 영역의 테이블 설계와 API 개발을 담당했습니다.

### 상품 (Products)
- 테이블 4개 설계 — 상품(soft delete), 카테고리·브랜드(트리 구조), 이미지, 파일명 시퀀스
- 상품 등록 · 수정 · 삭제 · 검색 API — 검색 조건(키워드·카테고리·가격·사이즈·컨디션)은 동적 쿼리로 처리
- 정렬 — 최신순, 인기순(`조회수 + 찜수 × 3`)
- 이미지 업로드 — 임시 저장 후 등록 확정 시 `PRD_001_1.jpg` 형식으로 파일명 정리
- 홈 화면 API — 신상품 / 인기 TOP / 랜덤 / 연관상품
- 상세 조회 쿼리 — `JOIN FETCH` 단일 쿼리로 N+1 문제 해결

### 찜 (Wishlist)
- 테이블 설계 — (회원, 상품) 복합 unique 제약으로 중복 찜 차단
- 찜 추가 · 해제, 마이페이지 찜 목록 API
- 찜 수 증감 쿼리 — DB에서 직접 `+1 / -1` 처리해 동시 요청에도 숫자 유지

### 관리자 (Admin)
- 상품 블라인드 · 복구 API — 사유와 함께 삭제 상태 전환, 복구 시 원복
- 신고 반려 · 제재 API — 제재 생성과 신고 상태 변경을 한 트랜잭션으로 처리
- 중복 제재 방지 — 처리 전 신고 상태 검사
