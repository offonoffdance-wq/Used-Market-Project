...
담당 기능 (Backend)
담당 도메인: Product · Wishlist · Review · Report — 테이블 설계 및 REST API 개발

상품 (Product)
테이블 4종 설계 — 상품(soft delete: 삭제 사유·일시 보관), 카테고리·브랜드(부모 FK 2단계 트리), 이미지(대표 이미지 정렬 순서), 파일명 시퀀스 채번 테이블
상품 등록·수정·삭제 — 삭제는 soft delete로 처리하고, 결제·배송 등 진행 중 거래가 있으면 삭제 차단
검색·필터 — 키워드·카테고리·가격·사이즈·컨디션을 조건부 동적 쿼리(파라미터가 없으면 해당 조건 제외)로 조합 + 페이징
정렬 — 최신순·가격순, 인기순은 조회수 + 찜수 × 3 점수로 계산 (구매 의도가 강한 찜에 가중치)
상세 조회 — 판매자·카테고리 계층·브랜드를 JOIN FETCH 단일 쿼리로 묶어 N+1 제거, 로그인 시 찜 여부까지 함께 반환
조회수 증가 — DB에서 직접 +1 UPDATE 처리해 동시 요청 시 유실 방지 + 세션으로 중복 조회 카운트 차단
이미지 업로드 — 업로드 시 임시 파일로 저장 후, 등록 확정 시점에 PRD_001_1.jpg(시퀀스_순번) 형식으로 rename·이동 → 등록 취소 시 잔여 파일 방지
홈 화면 API — 신상품 · 인기 TOP · 랜덤 · 연관 상품, 목록 썸네일은 배치 조회로 N+1 방지
(관리자) 상품 목록(다중 필터)·상세 조회, 블라인드(사유 기록)·복구
찜 (Wishlist)
중복 찜 차단 — (회원, 상품) 복합 unique 제약 + 등록 전 존재 검사로 이중 방어
찜 등록·취소·마이페이지 목록(페이징)
찜 수 동기화 — 상품의 찜 카운트를 DB에서 직접 +1 / -1 UPDATE로 갱신하고, 감소 시 0 미만 방지 가드를 둬 동시 요청에도 정합성 유지
리뷰 (Review)
리뷰 작성 — 배송 완료된 주문의 구매자 본인만 가능, 주문당 1개(사전 중복 검사 + UNIQUE 제약)
판매자 리뷰 목록 — 평균 별점 통계와 리뷰 목록을 함께 반환(비로그인 조회 가능)
상품 정보·썸네일 배치 조회로 N+1 방지
신고 (Report)
테이블 설계 — 처리 상태(접수 → 반려 / 처리완료) 흐름 관리, 신고 ID를 RPT_001 형식으로 애플리케이션에서 채번
신고 접수 · 내 신고 내역 조회(페이징)
(관리자) 신고 목록(키워드·사유·상태·기간 필터), 반려·제재 — 상태가 '접수' 일 때만 처리하도록 검사해 중복 제재 방지, 제재는 제재 생성 + 신고 상태 변경을 한 트랜잭션으로 원자 처리
