...
Spring Boot 기반의 중고거래 플랫폼입니다. 상품 등록부터 주문, 결제, 정산, CS까지 이커머스의 전체 트랜잭션 흐름을 3인 팀으로 설계·구현했으며, 저는 **상품(Product) 도메인과 관리자 상품·신고 처리의 백엔드**를 담당했습니다.
...
**상품(Product) 도메인의 백엔드**를 단독으로 설계·구현했고, 팀 공용 관리자 기능 중 **상품 관리·신고 처리**의 API와 비즈니스 로직을 담당했습니다.
