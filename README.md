...
| 영역 | 기능 | 설명 |
|---|---|---|
| **상품** | 테이블 설계 | 상품(soft delete)·카테고리/브랜드(트리)·이미지·파일명 시퀀스 |
| | 상품 CRUD | 등록·수정·삭제, 거래 진행 중이면 삭제 제한 |
| | 검색·필터 | 키워드·카테고리·가격·사이즈·컨디션 동적 쿼리 + 페이징 |
| | 정렬 | 최신순·가격순·인기순(조회수 + 찜수×3) |
| | 상세 조회 | JOIN FETCH로 N+1 제거 |
| | 조회수 | DB 직접 UPDATE, 세션 중복 방지 |
| | 이미지 업로드 | 임시 저장 → 등록 시 `PRD_001_1.jpg` 변환 |
| | 홈 화면 | 신상품·인기 TOP·랜덤·연관 상품 |
| | 관리자 | 상품 블라인드·복구 |
| **찜** | 찜 등록·취소·목록 | 마이페이지 목록 페이징 |
| | 중복 방지 | (회원, 상품) 복합 unique |
| | 찜 수 | DB 직접 +1 / -1 |
| **리뷰** | 리뷰 작성 | 배송완료 주문 구매자만, 주문당 1개 |
| | 판매자 리뷰 목록 | 평균 별점 + 목록 |
| **신고** | 신고 접수·조회 | 내 신고 내역 |
| | 관리자 처리 | 반려·제재 (상태 검사 후 트랜잭션 처리) |
| | 상태 관리 | 접수 → 반려 / 처리완료 |
