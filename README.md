...
## 담당 기능 (Backend)
**Product · Wishlist · Review · Report** 도메인 테이블 설계 및 API 개발

| 영역 | 기능 | 구현 내용 |
|---|---|---|
| **상품** | 테이블 설계 (4종) | 상품(soft delete), 카테고리·브랜드(부모 FK 트리 구조), 이미지, 파일명 시퀀스 |
| | 등록 · 수정 · 삭제 | 본인 확인 후 처리, 진행 중인 거래가 있으면 삭제 불가 |
| | 검색 · 필터 | 키워드·카테고리·가격·사이즈·컨디션 — 입력된 조건만 동적으로 조합 + 페이징 |
| | 정렬 | 최신순 · 가격순 · 인기순(`조회수 + 찜수 × 3`) |
| | 상세 조회 | 판매자·카테고리·브랜드를 JOIN FETCH 한 번에 조회 (N+1 방지) |
| | 조회수 | DB에서 직접 +1, 세션으로 중복 카운트 방지 |
| | 이미지 업로드 | 임시 저장 → 등록 확정 시 `PRD_001_1.jpg` 형식으로 파일명 정리 |
| | 홈 화면 | 신상품 · 인기 TOP · 랜덤 · 연관 상품 조회 API |
| | 관리자 | 상품 블라인드(사유 기록) · 복구 |
| **찜** | 테이블 설계 | (회원, 상품) 복합 unique로 중복 찜 차단 |
| | 등록 · 취소 · 목록 | 마이페이지 찜 목록 (페이징) |
| | 찜 수 관리 | 상품 찜 카운트를 DB에서 직접 ±1 — 동시 요청에도 수치 유지 |
| **리뷰** | 리뷰 작성 | 배송 완료 주문의 구매자만, 주문당 1개 (unique 제약) |
| | 판매자 리뷰 목록 | 평균 별점과 리뷰 목록 함께 반환 |
| **신고** | 접수 · 조회 | 신고 등록, 내 신고 내역 조회 |
| | 관리자 처리 | 반려·제재 — 미처리 상태만 처리(중복 제재 방지), 한 트랜잭션으로 묶음 |
## ⌘ 기술 스택
